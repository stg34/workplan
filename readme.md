...
Параметр `-e [кодировка]` или `--encoding [кодировка]` позволяет указать кодировку для чтения и записи файлов. По умолчанию используется UTF-8. Для Windows может потребоваться указать `-e cp1251` для корректной работы с кириллицей.
